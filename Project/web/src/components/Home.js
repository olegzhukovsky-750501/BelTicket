...
import { routes, route, busesByRoute, schedules } from '../Services'
...
class Home extends Component {
        ...
        if (type === 'from') {
            ...
            busesByRoute(selectedOption.route)
                .then(res => {
                    var options = [];
                    res.map((bus, i) => {
                        return options.push({ value: bus.name, label: bus.name, id: bbus._id })
                    })
    ...
    calculateFair = () => {
        ...
        if (this.state.to && this.state.from &&  this.state.qty) {
            var amount = Math.abs(this.state.to.fair - this.state.from.fair) * this.state.busClass.fairRatio * this.state.qty
    ...
    handleSubmit = event => {
        ...
        } else if (state.from && state.to && state.bus && state.busClass && state.time && state.qty && state.date) {
                ...
                <Row style={{ alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <Form.Row style={{ width: '75%' }}>
                        <Form.Group as={Col} controlId="from">
                            ...
                            <Select options={this.state.buses} onChange={this.handleChange("bus")} value={this.state.bus} />
                        </Form.Group>
                        <Form.Group as={Col} controlId="to">
                            <Form.Label>Class</Form.Label>
                            <Select options={this.state.classes} onChange={this.handleChange("busClass")} value={this.state.busClass} />
                        </Form.Group>
                    </Form.Row>
                    <Form.Row style={{ width: '75%' }}>
                        <Form.Group as={Col} controlId="from">
